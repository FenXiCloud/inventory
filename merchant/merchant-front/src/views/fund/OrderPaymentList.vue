<template>
  <div class="simple-page">
    <div class="simple-page__toolbar">
      <t-space break-line>
        <t-button v-auth="'orderPayment:edit'" theme="primary" style="border-radius: 4px" @click="addForm()">新 增</t-button>
        <t-button v-auth="'orderPayment:audit'" variant="outline" style="border-radius: 4px" @click="approved()">审 核</t-button>
        <t-button v-auth="'orderPayment:audit'" variant="outline" style="border-radius: 4px" @click="backApproved()">反审核</t-button>
        <t-button v-auth="'orderPayment:delete'" variant="outline" style="border-radius: 4px" @click="doRemove()">删 除</t-button>
        <t-select
        ...
        <template #ops="{ row }">
          <t-space size="small">
            <template v-if="row.orderStatus != '已审核'">
              <t-link v-auth="'orderPayment:edit'" theme="primary" @click="addForm('edit', row.id)">编辑</t-link>
              <t-link v-auth="'orderPayment:delete'" theme="danger" @click="doRemove(row)">删除</t-link>
            </template>
            <template v-else>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {colKey: 'businessNo', title: '源单编号', minWidth: 140, ellipsis: true},
        {colKey: 'supplierName', title: '供应商', minWidth: 120, ellipsis: true},
        {colKey: 'settlementAccount', title: '结算账户', minWidth: 120, ellipsis: true},
        {colKey: 'amount', title: '付款金额', width: 110, align: 'right'},
        {colKey: 'paymentMethodName', title: '付款方式', minWidth: 110, ellipsis: true},
        ...
        {colKey: 'collectionAmount', title: '本次预付款', width: 110, align: 'right'},
        {colKey: 'orderStaffName', title: '付款人', width: 90, align: 'center'},
  ...
  methods: {
    ...
    addForm(type = 'add', orderId = null) {
      this.closeTabKey('OrderPaymentForm');
      this.pushTab({
        keepAlive: false,
    ...
    approved() {
      ...
      DialogPlugin.confirm({
        body: `确定审核订单？`,
        onConfirm: () => {
          OrderPayment.approved('已审核', selectedRows).then((success) => {
            if (success !== false) {
    ...
    backApproved() {
      ...
      DialogPlugin.confirm({
        body: `确定反审核订单？`,
        onConfirm: () => {
          OrderPayment.approved('已保存', selectedRows).then((success) => {
            if (success !== false) {
