...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {colKey: 'collectionAmount', title: '付款金额', width: 110, align: 'right'},
  ...
  methods: {
    ...
    loadTotal() {
      OtherExpense.total(this.queryParams).then(({data}) => {
    ...
    approved() {
      ...
      DialogPlugin.confirm({
        content: `确定审核订单？`,
        onConfirm: () => {
          OtherExpense.approved('已审核', selectedRows).then((success) => {
            if (success !== false) {
    ...
    backApproved() {
      ...
      DialogPlugin.confirm({
        content: `确定反审核订单？`,
        onConfirm: () => {
          OtherExpense.approved('已保存', selectedRows).then((success) => {
            if (success !== false) {
