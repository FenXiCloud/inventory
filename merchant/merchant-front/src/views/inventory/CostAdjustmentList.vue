<template>
  <div class="simple-page">
    <div class="simple-page__toolbar">
      <t-space break-line>
        <t-button v-auth="'costAdjustment:edit'" theme="primary" style="border-radius: 4px" @click="addForm()">新 增</t-button>
        <t-button v-auth="'costAdjustment:audit'" variant="outline" style="border-radius: 4px" @click="approved()">审 核</t-button>
        <t-button v-auth="'costAdjustment:audit'" variant="outline" style="border-radius: 4px" @click="backApproved()">反审核</t-button>
        <t-select
        ...
        <template #ops="{ row }">
          <t-space size="small">
            <template v-if="editable(row)">
              <t-link v-auth="'costAdjustment:edit'" theme="primary" @click="addForm('edit', row.id)">编辑</t-link>
              <t-link v-auth="'costAdjustment:delete'" theme="danger" @click="doRemove(row)">删除</t-link>
            </template>
            <template v-else>
  ...
  methods: {
    ...
    approved() {
      ...
      DialogPlugin.confirm({
        ...
        onConfirm: () => {
          return CostAdjustment.approved('已审核', ids).then(() => {
    ...
    backApproved() {
      ...
      DialogPlugin.confirm({
        ...
        onConfirm: () => {
          return CostAdjustment.approved('已保存', ids).then(() => {
            MessagePlugin.success("操作成功~");
            this.clearSelection();
            this.loadList();
          });
    ...
    doRemove(row) {
      DialogPlugin.confirm({
        header: "系统提示",
        body: `确认删除：${row.orderNo || row.id}?`,
        onConfirm: () => {
          return CostAdjustment.remove(row.id).then(() => {
