<template>
  <div class="simple-page">
    <div class="simple-page__toolbar">
      <t-space break-line>
        <t-button v-auth="'otherInbound:edit'" theme="primary" style="border-radius: 4px" @click="addForm()">新 增</t-button>
        <t-button v-auth="'otherInbound:audit'" variant="outline" style="border-radius: 4px" @click="approved()">审 核</t-button>
        <t-button v-auth="'otherInbound:audit'" variant="outline" style="border-radius: 4px" @click="backApproved()">反审核</t-button>
        <t-select
        ...
        <template #ops="{ row }">
          <t-space size="small">
            <template v-if="editable(row)">
              <t-link v-auth="'otherInbound:edit'" theme="primary" @click="addForm('edit', row.id)">编辑</t-link>
              <t-link v-auth="'otherInbound:delete'" theme="danger" @click="doRemove(row)">删除</t-link>
            </template>
            <template v-else>
...
export default {
  ...
  methods: {
    ...
    addForm(type = 'add', otherInboundId = null) {
      const docName = this.presetType === '盘盈入库' ? '报溢单' : '其他入库单';
      this.pushTab({
        key: 'OtherInboundForm',
        title: type === 'edit' ? `编辑${docName}` : type === 'look' ? `查看${docName}` : `新增${docName}`,
        params: {type: type, otherInboundId: otherInboundId, inboundType: this.presetType}
      });
    },
    clearSelection() {
  ...
  created() {
    if (this.presetType) this.params.inboundType = this.presetType;
    this.loadDict();
    this.loadList();
  }
}
