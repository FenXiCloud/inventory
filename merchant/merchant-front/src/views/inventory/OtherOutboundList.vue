<template>
  <div class="simple-page">
    <div class="simple-page__toolbar">
      <t-space break-line>
        <t-button v-auth="'otherOutbound:edit'" theme="primary" style="border-radius: 4px" @click="addForm()">新 增</t-button>
        <t-button v-auth="'otherOutbound:audit'" variant="outline" style="border-radius: 4px" @click="approved()">审 核</t-button>
        <t-button v-auth="'otherOutbound:audit'" variant="outline" style="border-radius: 4px" @click="backApproved()">反审核</t-button>
        <t-select
        ...
        <template #ops="{ row }">
          <t-space size="small">
            <template v-if="editable(row)">
              <t-link v-auth="'otherOutbound:edit'" theme="primary" @click="addForm('edit', row.id)">编辑</t-link>
              <t-link v-auth="'otherOutbound:delete'" theme="danger" @click="doRemove(row)">删除</t-link>
            </template>
            <template v-else>
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {colKey: 'outboundType', title: '业务类型', width: 110, ellipsis: true},
        {colKey: 'totalAmount', title: '订单金额', width: 110, align: 'right'},
        {colKey: 'customerCode', title: '客户编码', width: 120, ellipsis: true},
