<template>
  <div class="simple-page">
    ...
    <div class="simple-page__toolbar">
      <t-space break-line>
        <t-button theme="primary" style="border-radius: 4px" v-auth="'purchaseReservation:edit'" @click="addForm()">新 增</t-button>
        <t-button variant="outline" style="border-radius: 4px" v-auth="'purchaseReservation:delete'" @click="batchDelete()">删 除</t-button>
        <t-button variant="outline" style="border-radius: 4px" v-auth="'purchaseReservation:audit'" @click="approved()">审 核</t-button>
        <t-button variant="outline" style="border-radius: 4px" v-auth="'purchaseReservation:audit'" @click="backApproved()">反审核</t-button>
        <t-select
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {colKey: 'supplierName', title: '供货商', width: 150},
        ...
        {colKey: 'statusText', title: '转采购状态', width: 100, align: 'center'},
  ...
  methods: {
    ...
    batchDelete() {
      ...
      DialogPlugin.confirm({
        ...
        onConfirm: () => {
          LoadingPlugin(true);
          const tasks = ids.map(id => PurchaseReservation.delete(id));
    ...
    approved() {
      ...
      DialogPlugin.confirm({
        ...
        onConfirm: () => {
          return PurchaseReservation.approved(ids, '已审核').then(() => {
    ...
    backApproved() {
      ...
      DialogPlugin.confirm({
        ...
        onConfirm: () => {
          return PurchaseReservation.approved(ids, '已保存').then(() => {
            MessagePlugin.success("操作成功~");
            this.clearSelection();
            this.loadList();
          })
    ...
    loadList() {
      this.loading = true;
      PurchaseReservation.list(this.queryParams).then(({data: {results, total}}) => {
