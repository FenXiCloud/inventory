...
export default {
  ...
  methods: {
    ...
    doPrint() {
      ...
      openPrint('采购退货单', {
        header: {
          ...
          amount: this.form.refundAmount ?? this.form.totalAmount,
        },
        items,
      });
    },
    ...
    selectPurchaseOrder() {
      ...
      let dialogId = openDrawer({
        ...
        closeBtn: false,
        size: '1200px',
        body: h(PurchaseReturnOrderSelect, {
  ...
  created() {
    LoadingPlugin(true);
    Promise.all([Supplier.select(), Warehouse.select(), Product.select()]).then((results) => {
