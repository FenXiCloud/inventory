...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {colKey: 'returnDate', title: '退货日期', width: 120, align: 'center'},
        {colKey: 'orderNo', title: '订单编号', minWidth: 160, ellipsis: true},
        {colKey: 'purchaseInboundNos', title: '关联入库单', minWidth: 140, ellipsis: true},
        {colKey: 'supplierName', title: '供货商', minWidth: 120, ellipsis: true},
        {colKey: 'refundTotalAmount', title: '退货金额', width: 110, align: 'right'},
        {colKey: 'discountAmount', title: '折扣金额', width: 110, align: 'right'},
        {colKey: 'refundAmount', title: '本次退款', width: 110, align: 'right'},
  ...
  computed: {
    ...
    footData() {
      ...
      return [{
        ops: '合计',
        refundTotalAmount: sum('refundTotalAmount'),
  ...
  methods: {
    ...
    addForm(type = 'add', orderId = null) {
      this.pushTab({
        key: 'PurchaseReturnForm',
        title: type == 'edit' ? '编辑采购退货单' : '新增采购退货单',
    ...
    loadList() {
      this.loading = true;
      PurchaseReturn.list(this.queryParams).then(({data: {results, total}}) => {
    ...
    doRemove(row) {
      DialogPlugin.confirm({
        ...
        onConfirm: () => {
          PurchaseReturn.remove(row.id).then(() => {
