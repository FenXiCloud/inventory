  ...
  <div class="page-column">
    ...
    <div class="page-column-footer modal-column-between bg-white-color border">
      ...
      <div>
        ...
        <!-- 已审核状态：转进货预订、转销售单 -->
        <t-button theme="success" @click="convertToPurchase()" :loading="loading"
          v-if="form.id && form.orderStatus === '已审核' && !isConvertedToPurchase">
          转进货预订
        </t-button>
        <t-button theme="warning" @click="convertToSalesOrder()" :loading="loading"
          v-if="form.id && form.orderStatus === '已审核' && !isConvertedToSalesOrder">
          转销售单
        </t-button>
        <!-- 反审核（仅已审核且未转进货时） -->
    ...
    <t-dialog
      ...
      header="选择供应商"
      :on-confirm="confirmConvertToPurchase"
      :on-close="() => showSupplierDialog = false"
    >
      <div class="p-16px">
        <p class="mb-12px">将根据此销售预订生成进货预订，请选择供应商：</p>
        <t-select
...
export default {
  ...
  computed: {
    ...
    flowCurrentStep() {
      if (this.isConvertedToSalesOrder) return 3;
      if (this.form.orderStatus === '已保存') return 0;
      if (this.form.orderStatus === '已审核' && this.isConvertedToPurchase) return 1;
      return 0;
    },
    columns() {
      return [
  ...
  methods: {
    ...
    saveOrder(type) {
      ...
      SalesReservation.save({
        salesReservation: Object.assign(this.form, {
          totalAmount: this.allFinalAmount
        }),
        salesReservationItemList: productData
      })
        .then((success) => {
          if (success) {
    ...
    loadProductsByCustomer() {
      if (this.form.customerId) {
        Customer.selectProduct(this.form.customerId)
          .then(({ data }) => {
    ...
    approved() {
      ...
      DialogPlugin.confirm({
        ...
        body: `确认审核该订单?`,
        onConfirm: () => {
          SalesReservation.approved(ids, '已审核').then(() => {
    ...
    unApproved() {
      ...
      DialogPlugin.confirm({
        ...
        body: `确认反审核该订单?`,
        onConfirm: () => {
          SalesReservation.approved(ids, '已保存').then(() => {
            MessagePlugin.success('操作成功~');
            this.loadFormData();
          });
        }
    ...
    confirmConvertToPurchase() {
      ...
      LoadingPlugin(true);
      SalesReservation.transferToPurchase(this.form.id, this.selectedSupplierId).then(({data}) => {
        MessagePlugin.success('已生成进货预订');
        this.$store.commit('pushTab', {
          key: 'PurchaseReservationForm',
          title: '进货预订详情',
          params: { orderId: data }
        });
      }).finally(() => LoadingPlugin(false));
    ...
    convertToSalesOrder() {
      DialogPlugin.confirm({
        header: '转销售单',
        body: `确认将该销售预订转为销售出库单?`,
        onConfirm: () => {
          SalesReservation.transferToSalesOrder(this.form.id).then(({data}) => {
            MessagePlugin.success('已生成销售出库单');
            this.$store.commit('pushTab', {
    ...
    loadFormData() {
      if (this.orderId) {
        SalesReservation.load(this.orderId).then(({data}) => {
          if (data) {
            CopyObj(this.form, data);
            this.customerId = data.customerId;
            this.isConvertedToPurchase = data.status > 0;
            this.isConvertedToSalesOrder = data.salesOrderId != null;
          }
          this.loadProductsByCustomer();
          this.productData = (data.salesReservationItemList || []).map((row) =>
    ...
    closeWindow() {
      ...
      this.$store.commit('newTab', 'SalesReservationList');
      this.$nextTick(() => {
  ...
  created() {
    LoadingPlugin(true);
    Promise.all([Customer.select(), Warehouse.select(), Supplier.select()])
      .then((results) => {
