...
public class PurchaseInboundController {
    ...
    public JsonResult importData(@RequestParam("file") MultipartFile multipartFile, @SaAccountVal AccountDto accountDto) {
        try {
            List<PurchaseInboundImportVo> rows = ImportVoUtil.readImportFile(multipartFile, PurchaseInboundImportVo.class);
            ...
            purchaseInboundService.importData(rows, accountDto.getMerchantId(), accountDto.getAccountBookId(), accountDto.getAdminId());
            return JsonResult.successful();
